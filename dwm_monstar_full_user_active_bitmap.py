...
if __name__ == '__main__':
    ...
    ( 
        ...
        (
            ...
            from 
                {dataset_gloom}.{table_id_login_src}
            where 
    ...
    ( 
        ...
        (
            select 
                json_extract_scalar(data, '$.uid')uid_login 
            from 
                {dataset_mons}.{table_id_login_src}
            where 
                date(datetime(time,'Asia/Singapore'))= '{tdy}'
            group by 
                json_extract_scalar(data, '$.uid')
        ...
        select *, '1'as bitmap
        from `{dataset_dst}.{table_id_reg_src}`
        where dtstatdate = '{tdy}' and reg_date = '{tdy}'
        and game_name = 'monstar'
    )
    '''.format(**ARGS)
